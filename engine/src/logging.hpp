...
Engine::Attempt::Result<P>& Engine::Attempt::Result<P>::fail(T msg, Args... args) {
	if (m_Status == Status::FAIL) Engine::Console::Error<P>("[ A ] <- "_D, msg, args...);
	return *this;
}
...
Engine::Attempt::Result<P> Engine::Attempt::Builder<P, Func>::does(T msg, Args... args) {
	Engine::Console::Log<P>("[ A ] -> "_D, msg, args...);
	...
	try {
		...
		Engine::Console::Error<P>("[ A ] <- "_D, e.what());
		final_status = Status::FAIL;
	}
	catch (...) {
		Engine::Console::Error<P>("[ A ] <- "_D, "Resulted in unknown Exception");
		final_status = Status::FAIL;
	}
